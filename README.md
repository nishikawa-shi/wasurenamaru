...
```bash
wasurenamaru$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
wasurenamaru$ sam local start-api
wasurenamaru$ curl http://localhost:3000/
```
...
```bash
wasurenamaru$ sam logs -n HelloWorldFunction --stack-name wasurenamaru --tail
```
...
```bash
aws cloudformation delete-stack --stack-name wasurenamaru
```
